<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jiaoyoutupian" label="交友图片" prop="jiaoyoutupian">
          <file-upload
          tip="点击上传交友图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jiaoyoutupian?ruleForm.jiaoyoutupian:''"
          @change="jiaoyoutupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jiaoyoutupian" label="交友图片" prop="jiaoyoutupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiaoyoutupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="交友目的" prop="jiaoyoumude">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="交友目的"
                  v-model="ruleForm.jiaoyoumude" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiaoyoumude" label="交友目的" prop="jiaoyoumude">
                    <span>{{ruleForm.jiaoyoumude}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingquaihao = true;
          ...
          if(o=='jiaoyoumude'){
            this.ruleForm.jiaoyoumude = obj[o];
	    this.ro.jiaoyoumude = true;
            continue;
          }
          if(o=='gerenjianjie'){
	    ...
	    this.ro.gerenjianjie = true;
          ...
          if(o=='jiaoyoutupian'){
            this.ruleForm.jiaoyoutupian = obj[o];
	    this.ro.jiaoyoutupian = true;
            continue;
          }
          if(o=='thumbsupnum'){
	...
	if(this.ruleForm.jiaoyoutupian!=null) {
		this.ruleForm.jiaoyoutupian = this.ruleForm.jiaoyoutupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaoyouxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaoyouxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    jiaoyoutupianUploadChange(fileUrls) {
	this.ruleForm.jiaoyoutupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
